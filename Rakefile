...
task :build_assets do
  ...
  %w[debian13 debian12 debian11 debian10].each do |platform|
    ...
    `docker run --platform linux/amd64 -v "$PWD/assets:/tmp/assets" ruby-plugin-#{platform} cp /assets/sensu-plugins-elasticsearch.tar.gz /tmp/assets/sensu-plugins-elasticsearch_#{version}_#{platform}_linux_amd64.tar.gz`
